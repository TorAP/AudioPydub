...
def callback(in_data, frame_count, time_info, flag):
    ...
    if effect == True:
        ...
        CF_samples_1 = combFilter(
            data, defaults["delay"], defaults["decay"], RATE)
        CF_samples_2 = combFilter(
            data, defaults["delay"] - 11.73, defaults["decay"] - 0.1313, RATE)
        CF_samples_3 = combFilter(
            data, defaults["delay"] + 19.31, defaults["decay"] - 0.2743, RATE)
        CF_samples_4 = combFilter(
            data, defaults["delay"] - 7.97, defaults["decay"] - 0.31, RATE)

        CF_sample_set = [CF_samples_1, CF_samples_2,
                         CF_samples_3, CF_samples_4]
        ...
        for i in range(len(data)):
            moist_mix[i] = ((1 - defaults["moist"]) * data[i]) + \
                (defaults["moist"] * postcomb_data[i])
